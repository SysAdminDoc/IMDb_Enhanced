...
- A score correction whose URL contains non-Latin characters is no longer stored past the length limit meant to bound it. The limit was applied to what you typed, and the address is percent-encoded afterwards, which can make it several times longer, so the ceiling the backup size is derived from described something nothing enforced. A correction that cannot fit is refused rather than trimmed, because a trimmed address points at the wrong page.
...
- In Windows High Contrast, a Seen mark and a Skip mark are told apart again. Both are drawn as an underline and the only difference between them was its colour, which that mode replaces, so the two looked the same while still suggesting there was something to read. They differ by the line now, dotted against wavy.
- The recovery page's primary button now has an edge you can find on a light background. It fills with the IMDb yellow and outlined itself in the same yellow, which measures 1.6:1 against a white page, so the button had no boundary at all there. It takes its outline from its own text colour instead.
...
- Settings, Data can compare two exported watchlist CSVs and list what appears on both, so two people can find something they would each watch. Both files are read on your device, neither is stored, and a file it cannot read is refused with the same message importing it would give. Titles match on IMDb id where a file carries one and on title and year where it does not.
- Settings, Data now draws a calendar of the days you watched something, a year at a time, from the dates already on your own marks. It uses the same rating colour scale you picked rather than a new one, places each day against the busiest day of that year, and states the count on every day. Nothing is fetched. A year with no dated viewings shows an empty calendar and says why.
- A media server or Servarr instance reachable over IPv6 loopback can be configured. A service bound to IPv6 only answers on `[::1]`, which was the one address the settings field refused, so the address the service itself prints was the one that would not work. Every other host is still refused, and all four places that decide whether a local request can leave now carry it, which a test enforces rather than assumes.
...
- Your own ratings page and your own lists index now get the same treatment as any other list: private Seen and Skip marks, the filters that narrow by them, dimming for low-rated titles, Copy as CSV, the runtime total, the roulette and the multi-search.
...
- The chart and episode surfaces are now rendered and measured under all five themes with marks present, rather than checked by reading the code. Every piece of injected text is held to WCAG AA against the colour actually painted behind it, and every badge is checked for staying inside its card. Nothing was found to be wrong, and the harness was proven able to find both kinds of defect before that was believed. A control lying over poster artwork is reported as unmeasurable rather than given a number computed against a background that is not there.
- Long lists are decorated in chunks rather than in one pass, so no single frame owns all the work. Measured on a 250-row chart in headless Chromium: the worst frame's script time was 31 and 33 milliseconds before and 28 and 29 after, against 23 for the same page with these features switched off entirely. So the decorators were never the 50-millisecond problem on this machine, and the change is worth about four of the ten milliseconds they add; what it buys is that the work no longer lands in one frame, which is what matters on a slower machine. A ten-row list takes the same single pass it always did.
...
- The extension's recovery and permissions pages follow your system colour scheme instead of forcing dark. A system set to light, or with no preference at all, now gets a light page rather than a dark one from an extension that offers a light theme everywhere else. Every control was checked for readable contrast in both, including the focus rings, which IMDb yellow could not carry on a light background.
...
- The private marks panel sorts, filters and pages. Sort by last viewing, title or state; narrow to Seen, Skip, note-only, or just the records carrying a note; rows come a hundred at a time with a count. Every row now says when the title was last watched, or when it was marked if it never was. A store at its 5,000-record limit draws the same hundred rows as a store of ten.
...
- A Letterboxd page that has changed shape now says so, instead of reporting that the film has no entry there. The two looked identical: a parser that quietly stops matching produces the same empty result as a film nobody has rated, which is how a similar extension came to show a blank rating for every film for months. When the page answers but carries none of the structure being read, the failure journal records it as a changed page and the widget says as much. An old cached score is not offered in that case, because the lookup worked and an old value would contradict it.
...
- A backup no longer puts your API keys on the clipboard. Export settings used to include every Radarr, Sonarr, Overseerr, Plex, Jellyfin, and Emby credential in plain text, and the clipboard is readable by other pages and remembered by clipboard managers. The normal export now leaves those out and tells you which ones it left out. Restoring such a backup keeps the credentials already on that device rather than blanking them. If you do need to move them, Export with credentials asks for a passphrase and encrypts the whole backup with it (PBKDF2-SHA256 at 600,000 iterations, the current OWASP recommendation, then AES-GCM, with a fresh salt and nonce every time). Importing one asks for the passphrase as soon as it recognizes the file, and a wrong passphrase or an altered file is refused before a single setting is written.
...
- "Grant access" now waits for the browser to finish opening the extension page before it reports success. Promise rejections and Firefox callback errors return a bounded failure instead of leaving an unhandled worker error, including when the recovery-tab fallback fails.
