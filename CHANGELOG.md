...
- The chart and episode surfaces are now rendered and measured under all five themes with marks present, rather than checked by reading the code. Every piece of injected text is held to WCAG AA against the colour actually painted behind it, and every badge is checked for staying inside its card. Nothing was found to be wrong, and the harness was proven able to find both kinds of defect before that was believed.
- Long lists are decorated in chunks rather than in one pass. On a 250-row chart in headless Chromium the script's own blocking time was 51, 77 and 58 milliseconds across three runs; chunked it is 48, 41 and 36, which puts it under the 50 millisecond mark where a click starts to feel dropped. The page's own rendering accounts for the rest: with these features off the same list still produces 70 to 91 millisecond frames. A ten-row list takes the same single pass it always did.
