...
${bridgeFor({ trusted:false })}\n${storeBody}\n})();\n`;
    ...
    fs.writeFileSync(path.join(storeDir, 'manifest.json'), `${JSON.stringify(storeManifest, null, 2)}\n`, 'utf8');
    fs.writeFileSync(path.join(storeDir, 'content.js'), storeContent, 'utf8');
    fs.writeFileSync(path.join(storeDir, 'boot.css'), bootCss, 'utf8');
    fs.writeFileSync(path.join(storeDir, 'recovery.js'), applyStoreProfile(recovery), 'utf8');
    STORE_COPIED_FILES.forEach(name => {
        fs.copyFileSync(path.join(extensionDir, name), path.join(storeDir, name));
    });
    const shipped = new Set([
        ...Object.values(storeManifest.icons || {}),
        ...Object.values(storeManifest.action?.default_icon || {}),
    ]);
    shipped.forEach(relative => {
    ...
    return { storeManifest, storeContent, excluded };
}
